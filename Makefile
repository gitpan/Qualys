# This Makefile is for the Qualys extension to perl.
#
...
#     ABSTRACT => q[connect to the Qualys scanner API]
#     AUTHOR => q[Anthony G Persaud (apersaud@gmail.com)]
#     NAME => q[Qualys]
#     PREREQ_PM => { LWP::UserAgent=>q[2], HTTP::Request=>q[0] }
#     VERSION_FROM => q[Qualys.pm]
#     clean => { FILES=>q[*.tar *.old pod2htm* *.htm*] }
...
TO_INST_PM = Qualys.pm

PM_TO_BLIB = Qualys.pm \
	$(INST_LIB)\Qualys.pm
...
Qualys.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Qualys\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  Qualys.pm $(INST_MAN3DIR)\Qualys.$(MAN3EXT) 
...
clean :: clean_subdirs
	-$(RM_RF) *.tar *.old pod2htm* *.htm* ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all $(INST_ARCHAUTODIR)/extralibs.ld perlmain.c tmon.out mon.out so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe perl perl$(EXE_EXT) $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def lib$(BASEEXT).def $(BASEEXT).exp $(BASEEXT).x core core.*perl.*.? *perl.core core.[0-9] core.[0-9][0-9] core.[0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9][0-9]
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(INST_LIB)\Qualys.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) "name:         Qualys" >> META.yml
	$(NOECHO) $(ECHO) "version:      0.01" >> META.yml
	$(NOECHO) $(ECHO) "version_from: Qualys.pm" >> META.yml
	$(NOECHO) $(ECHO) "installdirs:  site" >> META.yml
	$(NOECHO) $(ECHO) "requires:" >> META.yml
	$(NOECHO) $(ECHO) "    HTTP::Request:                 0" >> META.yml
	$(NOECHO) $(ECHO) "    LWP::UserAgent:                2" >> META.yml
...
TEST_FILE = test.pl
TEST_FILES = t\api.t t\attributes.t
TESTDB_SW = -d
...
ppd:
	...
	$(NOECHO) $(ECHO) "    <ABSTRACT>connect to the Qualys scanner API</ABSTRACT>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Anthony G Persaud (apersaud@gmail.com)</AUTHOR>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <DEPENDENCY NAME=\"HTTP-Request\" VERSION=\"0,0,0,0\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <DEPENDENCY NAME=\"LWP-UserAgent\" VERSION=\"2,0,0,0\" />" >> $(DISTNAME).ppd
